...
export class ConnectMySQLService {
  ...
  saveGiaovien(giaovien) {
    return this._httpClient.post(`${this.url}/add_giaovien.php`, giaovien,{ responseType: 'text' }).toPromise();
  };
  saveLop(lop) {
    return this._httpClient.post(`${this.url}/add_lop.php`, lop,{ responseType: 'text' });
  };
  delGiaoVien(id_giaovien:number){
    return this._httpClient.get(`${this.url}/delete_giaovien.php?id_giaovien=${id_giaovien}`,{responseType: 'text' });
  }
  delLop(id_lop:number){
    return this._httpClient.get(`${this.url}/delete_lop.php?id_lop=${id_lop}`,{responseType: 'text' });
  }
